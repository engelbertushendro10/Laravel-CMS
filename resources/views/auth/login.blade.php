...
<form method="POST" action="{{ route('login') }}">
    @csrf
        <div class="form-group">
            <label for="input_login_email" class="small mb-1">Email</label>
            <input name="email" value="{{ old('email') }}" type="email" class="form-control py-4 @error('email') is-invalid @enderror" placeholder="Enter Email" autocomplete="email"/>
            @error('email')
